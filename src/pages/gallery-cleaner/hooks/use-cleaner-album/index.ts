...
export function useCleanerAlbum(
  name: Exclude<GalleryCleanerAlbum, 'similarPhotos'>
): {
  uris: string[];
  status: GalleryCleanerAlbumStatus;
  refetch: () => void;
...
export function useCleanerAlbum(name: GalleryCleanerAlbum) {
  ...
  const uris = useSelector(
    store,
    (state) => state.context.albums[name]?.uris || []
  );
  const status = useSelector(
    store,
    (state) => state.context.albums[name]?.status || 'idle'
  );
  ...
  const isSelected = useMemo(() => {
    ...
    function isSelectedImpl(uri: string | string[]): boolean | 'mix' {
      if (Array.isArray(uri)) {
        ...
        return selectedCount === uri.length
          ? true
          : selectedCount > 0
            ? 'mix'
            : false;
