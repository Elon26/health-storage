...
export const store = createStore({
  ...
  on: {
    FETCH_ALBUM: (
      context,
      event: { albumName: GalleryCleanerAlbum },
      enqueue
    ) => {
      enqueue.effect(async () => {
        try {
          ...
          if (status !== 'granted') {
            ...
            if (res.status === MediaLibrary.PermissionStatus.DENIED) {
              throw new Error('Permission denied');
            }
          }
          ...
          if (albumName === 'similarPhotos') {
            ...
            uris = (await CleanerGalleryApiV2.fetchBlurryImages(3)).map(
              (asset) => asset.uri
            );
          } else {
            uris = (
          ...
          store.send({ type: 'SET_ALBUM_ERROR', albumName: event.albumName });
        }
      });
    ...
    REFETCH_ALL: (context, _event, enqueue) => {
      for (const album of GalleryCleanerAvailableAlbums) {
        if (
          context.albums[album].status === GalleryCleanerAlbumStatuses.fetched
        ) {
          enqueue.effect(() => {
    ...
    SET_ALBUM_ERROR: (
      ...
      return {
        ...
        albums: {
          ...context.albums,
          [event.albumName]: {
            ...context.albums[event.albumName],
            status: GalleryCleanerAlbumStatuses.error,
          },
        },
      };
    },
    ...
    SET_ASSETS: (
      context,
      event: { assets: Record<string, CleanerGalleryApiV2.AssetDetails> }
    ) => {
      return {
        ...context,
        assets: {
