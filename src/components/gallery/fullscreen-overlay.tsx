...
export function FullscreenViewOverlayComponent({
  ...
  return (
    <View style={{ flex: 1 }}>
      <View
        className="absolute z-20 h-[44] flex-row items-center justify-between gap-5 inset-x-0 px-4"
        style={{
          top: insets.top,
        }}
      >
      ...
      {withButtons && (
        <View
          className="absolute flex-row gap-2.5 inset-x-10 bottom-0"
          style={{ bottom: insets.bottom + scaleY(16) }}
        >
          <Pressable
            className="flex-1 items-center justify-center rounded-2xl border-2 border-primary bg-background/85 h-12"
            onPress={async () => {
          ...
          <Pressable
            className="flex-1 items-center justify-center rounded-2xl border-2 border-red bg-background/85 h-12"
            onPress={async () => {
              impactAsync(ImpactFeedbackStyle.Medium);
              deleteByUri?.(uri);
            }}
